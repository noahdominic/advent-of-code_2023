...
def main():
        ...
        with open("input-full.txt") as f: 
                hands, bids = zip(*[(handstr, int(bidstr)) for handstr, bidstr in map(lambda line: line.strip().split(), f.readlines())])
        ...
        print(sum((i + 1) * bid for i, bid in enumerate(bids_sorted)))
